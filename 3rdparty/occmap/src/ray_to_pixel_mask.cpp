...
std::vector<Eigen::Array2i> mapping::RayToPixelMask(
  ...
  int subpixel_scale)
{
  ...
  if (scaled_begin.x() > scaled_end.x()) {
    if (scaled_end.x() < 0) {return std::vector<Eigen::Array2i>(0);}
    return mapping::RayToPixelMask(scaled_end, scaled_begin, subpixel_scale);
  }
